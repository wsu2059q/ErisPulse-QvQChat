...
class QvQMemory:
    ...
    async def add_short_term_memory(
        ...
        memory_key = self._get_session_key(
    ...
    async def add_long_term_memory(
        self, user_id: str, content: str, tags: List[str] = None
    ) -> None:
        """
        添加长期记忆
    ...
    async def add_group_memory(
        ...
        if is_context:
            ...
            if len(memory["sender_memory"][sender_id]) > 10:
                memory["sender_memory"][sender_id] = memory["sender_memory"][sender_id][
                    -10:
                ]
    ...
    async def _check_and_compress_memory(self, user_id: str) -> None:
        ...
        if long_term_count >= compression_threshold:
            ...
            if self.ai_manager:
                ...
                if ai_client:
                    self.logger.info(
                        f"记忆数量({long_term_count})达到阈值({compression_threshold})，开始压缩用户 {user_id} 的记忆"
                    )
...
4. 返回JSON格式的记忆列表"""
        ...
        try:
            ...
            try:
                ...
                memory["long_term"] = [
                    ...
                    for entry in (
                        compressed if isinstance(compressed, list) else [compressed]
                    )
                ]
                await self.set_user_memory(user_id, memory)
                return "记忆已成功压缩"
    ...
    async def get_memory_summary(
        ...
        if group_id:
            ...
            summary += (
                f"群聊记忆: {sender_count} 条发送者记忆, {context_count} 条共享上下文\n"
            )
    ...
    async def export_memory(
        ...
        if group_id:
            ...
            export_data["session_history"] = await self.get_session_history(
                user_id, group_id
            )
        else:
